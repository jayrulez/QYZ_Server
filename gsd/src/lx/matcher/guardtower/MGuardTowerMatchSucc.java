		...
		new xdb.Procedure() {
			...
			protected boolean process() {
				...
				team.members.forEach(roleShowInfo4 -> {
					if(Roleinfos.select(roleShowInfo4.roleid) != null){
						roleidSet.add(roleShowInfo4.roleid);
					}
				});
				final int countDown = CfgMgr.teamfight.matchsucccountdown;
				final SGuardTowerMatcherUpdate re = new SGuardTowerMatcherUpdate(1, team, countDown);
				for(long roleid : roleidSet) {
					FCondition.checkA(roleid, CfgMgr.guardtower.dailylimit, 1, By.Open_Ectype, ConfigId.GUARD_TOWER_ECTYPE, 0);
					FMap.succMatch(roleid);
				}
				xdb.Transaction.tsendWhileCommit(roleidSet, re);
