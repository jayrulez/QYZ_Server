...
import lx.gs.map.msg.SGuardTowerMatchCancel;
import lx.gs.map.msg.SGuardTowerMatchMemberOut;
	...
	protected void process() {
		new xdb.Procedure() {
			...
			protected boolean process() {
				if(err == 0) {
					for (long roleid : cancelmembers)
						FMap.cancelMatch(roleid);
				}
				if(teammembers.equals(cancelmembers)){
					Onlines.getInstance().multicast(teammembers, new SGuardTowerMatchCancel());
				} else {
					long cancelRole = cancelmembers.get(0);
					Onlines.getInstance().send(cancelRole, new SGuardTowerMatchCancel());
					teammembers.remove(Long.valueOf(cancelRole));
					teammembers.forEach(aLong -> Onlines.getInstance().send(cancelRole, new SGuardTowerMatchMemberOut(cancelRole)));
				}
				return true;
			}
		}.execute();	}
