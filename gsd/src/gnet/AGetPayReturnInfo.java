	...
	protected void process() {
        new xdb.Procedure() {
            ...
            protected boolean process() {
                final AuanyToGs ctx = (AuanyToGs)getContext();
                // 如果auany标记已经被本角色领取,但本角色未领取,说明上次操作未完成,重置为未领取
                final xbean.RolePay info = FPay.getRolePay(ctx.roleid);
                final boolean hasgot;
                hasgot = !(totalpay > 0 && !info.getHasgotpayreturn() && (getreturnroleid == 0 || ctx.roleid == getreturnroleid));
                info.setHasgotpayreturn(hasgot);
                info.setTotalyunbao(totalyuanbao);
                info.setTotalbindyuanbao(totalbindyuanbao);
                info.setTotalvipexp(totalvipexp);
                Onlines.getInstance().send(ctx.roleid, FPay.create(hasgot, totalyuanbao, totalbindyuanbao, totalvipexp));
                return true;
            }
        }.execute();
	}
