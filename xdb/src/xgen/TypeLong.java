...
public class TypeLong extends Type {
	...
	public void verifyForeign(Xdb xdb, Foreign foreign) {
		foreign.throwIf(null != foreign.getKey(), "[long] need value only.");
		if (null != foreign.getValue()) {
			Table table = xdb.getTable(foreign.getValue());
			foreign.throwIf(null == table, "[long] table not exist.");
			foreign.throwIf(table.isMemory(), "[long] foreign table is memory");
			foreign.throwIf(table.getKeyType() != this, "[long] type not match");
		}
	}
	...
	public void print4reset(XBean bean, Variable var, PrintStream ps, String prefix) {
		String initial = var.getInitial().isEmpty() ? "0L" : var.getInitial();
		ps.println(prefix + var.getname() + " = " + initial + ";");
	}

	@Override
	public void unmarshal(XBean bean, Variable var, PrintStream ps, String prefix, String localvarname) {
		ps.println(prefix + localvarname + " = _os_.unmarshal_long();");
	}
	...
	public void setterInterface(XBean bean, Variable var, PrintStream ps, String prefix) {
		ps.println(prefix + "public void set" + var.getName() + "(long _v_); // " + var.getComment());
	}
	...
	public void setter(XBean bean, Variable var, PrintStream ps, String prefix) {
		...
		ps.println(prefix + "			return new xdb.logs.LogLong(this, " + var.getname() + ") {");
