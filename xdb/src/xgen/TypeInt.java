...
public class TypeInt extends Type {
	...
	public void verifyForeign(Xdb xdb, Foreign foreign) {
		foreign.throwIf(null != foreign.getKey(), "[int] need value only.");
		if (null != foreign.getValue()) {
			Table table = xdb.getTable(foreign.getValue());
			foreign.throwIf(null == table, "[int] table not exist.");
			foreign.throwIf(table.isMemory(), "[int] foreign table is memory");
			foreign.throwIf(table.getKeyType() != this, "[int] type not match");
		}
	}
	...
	public void unmarshal(XBean bean, Variable var, PrintStream ps, String prefix, String localvarname) {
		ps.println(prefix + localvarname + " = _os_.unmarshal_int();");
	}
	...
	public void getterInterface(XBean bean, Variable var, PrintStream ps, String prefix) {
		ps.println(prefix + "public int get" + var.getName() + "(); // " + var.getComment());
	}
	...
	public void setterInterface(XBean bean, Variable var, PrintStream ps, String prefix) {
		ps.println(prefix + "public void set" + var.getName() + "(int _v_); // " + var.getComment());
	}
	...
	public void setter(XBean bean, Variable var, PrintStream ps, String prefix) {
		...
		ps.println(prefix + "			return new xdb.logs.LogInt(this, " + var.getname() + ") {");
