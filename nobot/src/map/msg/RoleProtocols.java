...
public class RoleProtocols extends __RoleProtocols__ {
	...
	protected void process() {
	    try {
            final Stub stub = ((Coder) getManager().getCoder()).getStub(ptype);
            final xio.Protocol p = stub.newInstance();
            p.unmarshal(new OctetsStream(data));
            p.setContext(getContext());
            p.setConnection(getConnection());
            p.dispatch(stub);
        } catch (Exception e) {
            Trace.error("RoleProtocol. roleid:" + roleid + ",ptype:" + ptype, e);
        }
	}
