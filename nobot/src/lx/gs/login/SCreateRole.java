...
public class SCreateRole extends __SCreateRole__ {
	...
	protected void process() {
		...
		if(err != 0) {
            Trace.error("SCreateRole fail. account:{} userid:{} retcode:{}", ctx.account, ctx.userid, err);
        } else {
            Trace.info("SCreateRole succ. account:{} userid:{} roleid:{}", ctx.account, ctx.userid, newinfo.roleid);
            new CRoleLogin(newinfo.roleid).send(getConnection());
        }
	}
