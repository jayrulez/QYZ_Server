...
public class SGetRoleList extends __SGetRoleList__ {
	...
	protected void process() {
        ...
        if(roles.isEmpty()) {
            Trace.info("acount:{} userid:{} rolelist is empty. create it!", ctx.account, ctx.userid);
            final CCreateRole re = new CCreateRole();
            re.name = ctx.account;
            re.profession = EProfessionType.QINGYUNMEN;
            re.gender = common.Utils.randomChoose(GenderType.enums);
            re.send(getConnection());
        } else {
            new CRoleLogin(roles.get(0).roleid).send(getConnection());
        }
	}
