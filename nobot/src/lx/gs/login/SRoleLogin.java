...
import lx.gs.CPing;
import lx.gs.chat.msg.CChat;
import lx.gs.team.msg.CCreateTeam;
import robot.Ctx;
import xdb.Trace;
...
public class SRoleLogin extends __SRoleLogin__ {
	...
	protected void process() {
        ...
        if(err != 0) {
            Trace.error("SRoleLogin fail. account:{} roleid:{}", ctx.account, ctx.roleid);
        } else {
            ctx.roleid = roledetail.roleid;
            ctx.roleDetail = roledetail;
            if(roledetail.level < 30) {
                final CCreateTeam ct = new CCreateTeam();
                ct.send(getConnection());
                TaskQueue.getScheduleExecutor().schedule(() -> {
                    final CChat c = new CChat();
                    c.channel = ChannelType.TEAM;
                    c.text = "\\role setlevel 90";
                    c.send(getConnection());
                }, 1, TimeUnit.SECONDS);
            }
            Trace.info("SRoleLogin succ. account:{} userid:{} roleid:{}", ctx.account, ctx.userid, ctx.roleid);
            TaskQueue.getScheduleExecutor().scheduleAtFixedRate(() -> {
                if(ctx.roleid > 0) {
                    new KeepAlive().send(getConnection());
                    new CPing().send(getConnection());
                }
