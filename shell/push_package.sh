...
backup=${FULL_NAME}.bak
if [[ ! -d $backup ]]; then
	mkdir $backup
fi
mv $FULL_NAME $backup/$date
isok $?  "mv $FULL_NAME $backup/$date"
svn export svn://diskstation/luxian/trunk/serverbin $FULL_NAME > /dev/null
isok $? "svn export svn://diskstation/luxian/trunk/serverbin $FULL_NAME > /dev/null"
cd $FULL_NAME
chmod 755 *.py
chmod 755 *.sh
